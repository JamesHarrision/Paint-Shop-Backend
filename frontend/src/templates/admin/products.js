...
export const AdminProductsTemplate = () => `
    <div class="flex flex-col md:flex-row justify-between items-center mb-12 md:mb-16 gap-8 md:gap-12">
        <div class="text-center md:text-left">
            <h2 class="text-4xl md:text-6xl font-black uppercase tracking-tighter">Quản trị <span class="bg-[#20B2AA] text-white px-2 border-2 border-black">Sản phẩm</span></h2>
            <p class="text-[10px] md:text-sm font-black uppercase tracking-widest text-gray-500 mt-6 italic border-l-4 border-black pl-4">Kho hàng: <span id="admin-product-count" class="text-black bg-[#E2725B] text-white px-2">...</span> mặt hàng</p>
        </div>
        <div class="flex flex-col gap-4 md:gap-6 w-full md:w-auto">
            ...
            <button onclick="window.showAddProductModal()" class="btn-brutal bg-[#20B2AA] text-white w-full text-xs !py-3">+ THÊM SẢN PHẨM MỚI</button>
        </div>
    </div>
    ...
    <div id="add-product-modal" class="fixed inset-0 z-[10002] hidden">
        ...
        <div class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-2xl bg-white border-4 border-black shadow-[8px_8px_0px_#20B2AA] md:shadow-[12px_12px_0px_#20B2AA] p-8 md:p-12 max-h-[90vh] overflow-y-auto animate-fade-in">
            <div class="flex justify-between items-center mb-8 md:mb-10 border-b-4 border-black pb-6">
                <h2 id="product-modal-title" class="text-3xl md:text-4xl font-black uppercase tracking-tighter italic">Quản lý <span class="bg-[#E2725B] text-white px-2 not-italic">Product</span></h2>
            ...
            <form id="add-product-form" class="space-y-6">
                ...
                <button type="submit" id="btn-product-submit" class="btn-brutal bg-[#E2725B] text-white w-full text-lg md:text-xl mt-8">ĐĂNG SẢN PHẨM NGAY</button>
            </form>
        </div>
    </div>
`;
